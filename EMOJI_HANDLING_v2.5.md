...
```bash
python nxos_compliance_checker_v2.5.py config.txt --format csv
```
...
```bash
python nxos_compliance_checker_v2.5.py config.txt --format syslog
```
...
```bash
python nxos_compliance_checker_v2.5.py config.txt | grep "PASS"
```
...
```bash
python nxos_compliance_checker_v2.5.py config.txt | sed 's/✅/[PASS]/g; s/❌/[FAIL]/g; s/⚠️/[WARN]/g'
```
