...
```json
{
  "metadata": {
    ...
    "policy": {
      "policy_name": "Cisco NX-OS Security Hardening - v2.4-clean",
      "version": "2.4-clean"
    },
...
```bash
# Check device 1
python template_compliance_checker_v2_4_clean.py device1.txt \
    --format json --output device1_results.json

# Check device 2
python template_compliance_checker_v2_4_clean.py device2.txt \
...
python template_compliance_checker_v2_4_clean.py $CONFIG_FILE \
    --format json --output results.json
...
python template_compliance_checker_v2_4_clean.py config.txt --verbose 2>&1 | grep "NX-MP-005AB"
```
...
python template_compliance_checker_v2_4_clean.py config.txt --verbose 2>&1 | grep "AAA Configuration"
```
...
python template_compliance_checker_v2_4_clean.py backup_config.txt \
...
```
┌─────────────────────────────────────────────────────────────┐
│           v2.4-clean Quick Reference                        │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Basic Check:                                               │
│    python template_compliance_checker_v2_4_clean.py \       │
│        config.txt                                           │
│                                                             │
│  With Custom Policy:                                        │
│    python template_compliance_checker_v2_4_clean.py \       │
...
- Review `MIGRATION_GUIDE_V2_4_CLEAN.md`
- Check `CHANGELOG_V2_4_CLEAN.md`
