#!/usr/bin/env python3
"""
Production-Quality Template Compliance Checker v2.4-clean
==========================================================================

Enterprise-grade compliance checker with unified value checking approach.
All value checks (absolute values and ranges) use the same check_value method
with backward compatibility for existing policy files.
...
class ProductionComplianceChecker:
    ...
    def __init__(self, config_file: str, policy_file: str = None, verbose: bool = False):
        ...
        if policy_file is None:
            policy_file = Path(__file__).parent / "nxos_compliance_policy_v2_4_clean.yaml"
    ...
    def check_bgp_log_neighbor_changes_all_vrfs(self, check: Dict) -> Tuple[str, str]:
        ...
        - This checks VRFs that have BGP configuration (under "router bgp")
        - It does NOT check system VRFs without BGP (e.g., management VRF)
        - Only VRFs with "vrf <name>" under "router bgp" need log-neighbor-changes
        
        Logic:
        1. BGP is configured (router bgp <ASN>)
        2. log-neighbor-changes is present in the default/global BGP context
        3. For each VRF configured under BGP (vrf <name>), log-neighbor-changes is also present
        
        Returns:
            Tuple[str, str]: (status, message)
            - PASS: log-neighbor-changes configured in all BGP VRFs
            - FAIL: log-neighbor-changes missing in default or any BGP VRF
        """
        ...
        bgp_obj = bgp_objs[0]  # Should only be one router bgp statement
        ...
        # Find all VRF configurations under BGP (not system VRFs, only BGP VRFs)
        # Pattern: "  vrf <name>" (indented under router bgp)
        vrf_objs = bgp_obj.re_search_children(r'^\s+vrf\s+\S+')
        
        if not vrf_objs:
            # No VRFs configured under BGP, only default VRF has BGP
        ...
        for vrf_obj in vrf_objs:
            vrf_name = vrf_obj.text.strip().split()[1]  # Extract VRF name
            
            # Check if this VRF has log-neighbor-changes
        ...
        if vrfs_missing_log:
            # FAIL if ANY BGP VRF is missing log-neighbor-changes (changed from WARN in v2.5)
            missing_list = ', '.join(vrfs_missing_log[:5])  # Show first 5
    ...
    def execute_check(self, check: Dict) -> Dict:
        ...
        check_methods = {
            ...
            'value': self.check_value,  # Unified value checking for all value types
            'value_range': self.check_value,  # Backward compatibility - alias to check_value
        ...
        # Get weight from policy or use defaults
        ...
        if status == 'PASS':
            ...
            score = weight // 2  # Partial credit for warnings
            self.warning_count += 1
        elif status == 'INFO':
            score = weight  # Don't penalize for N/A checks
            self.info_count += 1
        else:  # ERROR
            score = 0
            self.error_count += 1
    ...
    def run_all_checks(self) -> List[Dict]:
        ...
        print(f"\n{'='*80}")
        print(f"Production Compliance Checker v2.4-clean (Unified Value Checking)")
        print(f"{'='*80}")
        ...
        for category_key, checks in check_categories:
            ...
            for check in checks:
                ...
                # Print real-time status if verbose
                if self.verbose:
                    status_icon = {
                        'PASS': '✅ [PASS]', 
                        'FAIL': '❌ [FAIL]', 
    ...
    def get_grade(self) -> str:
        ...
        # Get thresholds from policy or use defaults
    ...
    def export_json(self, output_file: str):
        ...
        # Create clean results without emojis for JSON export
        clean_results = []
        for result in self.results:
            clean_result = result.copy()
            # Strip emojis from message field
            clean_result['message'] = self._strip_emojis(result['message'])
            # Strip emojis from name field if present
    ...
    def export_csv(self, output_file: str):
        ...
        with open(output_file, 'w', newline='') as f:
            ...
            writer.writeheader()
            for result in self.results:
                # Create clean row without emojis
                clean_row = {
                    'id': result['id'],
    ...
    def export_structured_log(self, output_file: str = None):
        ...
        def format_log_line(result: Dict) -> str:
            def escape_value(v):
                if v is None:
                    return ""
                # Strip emojis from value before escaping
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Production-Quality Template Compliance Checker v2.4-clean (Unified Value Checking)',
...
Backward Compatibility:
  This version supports both old and new policy file formats:
  - Old: check_type: value_range (automatically converted)
  - New: check_type: value with comparison: range
        """
    )
